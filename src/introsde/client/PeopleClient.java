...
public class PeopleClient{
	...
	private Long first_person_id = (long)-1;
	private Long last_person_id = (long)-1;
	Measure measure_element_first_id = null;
	...
	private void readPersonList() {
		...
		List<Person> p_list = people.readPersonList();
	...
	private void updatePerson() {
		...
		String response = people.updatePerson(p);
		System.out.println("RESPONSE:");
		String res = (response.equals("Person update")) ? "OK" : "ERROR";
	...
	private void createPerson() {
		...
		printPerson(p);
		printMeasure(m);
		Long idPerson = people.createPerson(p, measure);
		System.out.println(" ");
		System.out.println("RESPONSE:");
	...
	private void deletePerson() {
		...
		Holder<Long> id = new Holder<Long>(last_person_id);
	...
	private void readPersonHistory() {
		...
		System.out.println(" ");
		List<Measure> history = people.readPersonHistory(first_person_id, measureType);
	...
	private void readPersonMeasure() {
		...
		Measure measure_first_person = people.readPerson(first_person_id).getCurrentHealth().getMeasure().get(0);
	...
	private void updatePersonMeasure() {
		...
		System.out.println("id: "+first_person_id);
		printMeasure(measure_element_first_id);
		measure_element_first_id.setMeasureValue("50");
		System.out.println(" ");
		System.out.println("RESPONSE:");
		Holder<Measure> measure = new Holder<Measure>(measure_element_first_id);
	...
	public void printCurrentHealth(CurrentHealth ch) {
		List<Measure> list_measure = ch.getMeasure();
